...
const predictChoices: ChoiceCheckpointOption[] = [
  {
    id: "trace-one",
    label: "`1`",
    feedback: "Only one loop executes when we check `while score < 1` and then add one.",
  },
  {
    id: "trace-two",
    label: "`2`",
    feedback: "Try again. With this script, there are three loop checks before stop.",
  },
  {
    id: "trace-three",
    label: "`3`",
    feedback: "Correct. Starting from 0, the loop runs 3 times when the bound is `< 3`.",
  },
];
    ...
    >
      ...
      <section id="loop-iteration" className="lesson-section guided-topic mission-topic">
        ...
        <ChoiceCheckpoint
          ...
          question="How many checks should the sample script run before ending?"
          options={predictChoices}
          correctId="trace-three"
          successMessage="Right. The checks are logged at each guard evaluation and the loop stops once the guard fails."
          hint="After each check, score increases by 1 from 0 to 1, then 1 to 2, then 2 to 3."
        />
      </section>
