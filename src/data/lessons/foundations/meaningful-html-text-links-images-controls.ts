...
export const meaningfulHtmlTextLinksImagesControlsLesson = {
  ...
  guidedSteps: [
    ...
    },
    {
      ...
      blocks: [
        ...
        { type: "ordering-checkpoint", activityId: "order-labeled-control" },
      ],
  ...
  activities: [
    ...
    },
    {
      ...
      title: "Build a labelled control",
      prompt: "Arrange the decisions that produce a clear label-control association.",
      items: [
        { id: "match-values", label: "Give the label's `for` and input's `id` the same value." },
        { id: "choose-name", label: "Write a visible label that names the expected information." },
        { id: "choose-input", label: "Choose an input type appropriate for the expected information." },
        { id: "verify", label: "Verify that selecting the label focuses or activates the intended control." },
      ],
      correctOrder: ["choose-name", "choose-input", "match-values", "verify"],
      successMessage: "Correct. The visible name and matching association create a usable labelled control.",
      errorMessage: "First decide what the control asks for, then choose the control, associate matching values, and finally verify the behavior.",
    },
  ],
  completionRule: {
